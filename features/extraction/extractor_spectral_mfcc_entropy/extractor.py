...
PATH_ARFF_OUT = "./results/MFCC_ENTROPY_SPECTRAL.arff"
...
class Song:
    def __init__(self, s_type="", name=""):
        ...
        # mean_s_centroid, std_s_centroid, mean_s_rolloff, std_s_rolloff, mean_s_flux, std_s_flux
    ...
    @spectal_features.setter
    def spectral_features(self, value):
        self._spectral_features = value;
        ...
        for m in self.mfcc_features:
            for i in range(0, N_MFCC):
                ...
                if (k != 1 and k != 2):
                    s += format(m[i], '.6f') + ","
            k = k + 1;
        for f in self._spectral_features:
            s += format(f, '.6f') + ","
        s += self.type
        return s
...
def extract_spectral_features(song):
    y = song.wav
    sr = song.sr
    ...
    song.spectral_features = (np.mean(cent), np.std(cent), np.mean(roll), np.std(roll), np.mean(flux), np.std(flux))
